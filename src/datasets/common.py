...
def get_dataloader(dataset, is_train, args, image_encoder=None):
    if image_encoder is not None:
        ...
        if args.basis and args.subsample_eval:
            is_val = all(dataset.endswith('Val') for dataset in args.eval_datasets)
            if is_val:
                dataset = dataloader.dataset
                
                T = len(args.eval_datasets)
                data_len = len(dataset)
                indices = list(range(data_len))
                random.seed(args.seed)
                match = re.search(r"_M(\d+)_", args.basis)
                M = int(match.group(1))
                sampled_indices = random.sample(indices, int(M/T*data_len))
                dataloader = DataLoader(
                    dataset,
                    batch_size=args.batch_size,
                    num_workers=args.num_workers,
                    sampler=SubsetSampler(sampled_indices)
                )
