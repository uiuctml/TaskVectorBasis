...
@dataclass(frozen=True)
class OurMultiHeadInputFeatures(InputFeatures):
    """
    Inherit from Transformers' InputFeatuers.
    """
...
class CombinedDataset(torch.utils.data.Dataset):
    def __init__(self, datasets):
        """
        Args:
            datasets (list): A list of datasets to combine.
        """
        self.datasets = datasets
        self.dataset_indices = []  # Stores indices to know which dataset a sample comes from
        ...
        self.total_len = sum(len(dataset) for dataset in datasets)
    
    def __len__(self):
        return self.total_len
    
    def __getitem__(self, idx):
        # Identify which dataset the idx corresponds to
        dataset_idx = self.dataset_indices[idx]
        
        # Adjust idx to access the corresponding dataset
        adjusted_idx = idx - sum(len(d) for d in self.datasets[:dataset_idx])
        
        # Fetch the sample from the appropriate dataset
        sample = self.datasets[dataset_idx][adjusted_idx]
        
        inputs = sample 

        # Add dataset identifier as a Tensor
        # dataset_idx_tensor = torch.as_tensor(dataset_idx, dtype=torch.long)
        inputs.__dict__['task_idx'] = dataset_idx
        return inputs